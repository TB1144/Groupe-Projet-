...
$pageTitle = 'Créer une offre — Web4All';
...
<main class="page-container">
    <section class="login-container">
        <div class="login-box">
            ...
            <form method="POST" action="/offres/creer">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'], ENT_QUOTES, 'UTF-8') ?>">

                <div class="input-group">
                    <label for="titre">Titre <span class="required">*</span></label>
                    <input type="text" id="titre" name="titre" required
                           value="<?= htmlspecialchars($data['titre'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                </div>

                <div class="input-group">
                    <label for="description">Description <span class="required">*</span></label>
                    <textarea id="description" name="description" rows="5" required><?= htmlspecialchars($data['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
                </div>
                    ...
                    <select id="id_entreprise" name="id_entreprise" required>
                        ...
                        <?php foreach ($entreprises as $ent): ?>
                            <option value="<?= (int)$ent['id'] ?>"
                                <?= (int)($data['id_entreprise'] ?? 0) === (int)$ent['id'] ? 'selected' : '' ?>>
                ...
                <div class="input-group">
                    ...
                    <input type="date" id="date_offre" name="date_offre" required
                           value="<?= htmlspecialchars($data['date_offre'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
                </div>
